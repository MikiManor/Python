...
def get_Csv_Date(csvFileName):
    original_data = pd.read_csv("ModifiedData.csv",
                                names=[
...
def split_Data_To_Test_And_Train_Sets(encoded_data):
    X_train, X_test, y_train, y_test = skl.model_selection.train_test_split(encoded_data[encoded_data.columns - ["Target"]],
...
def logical_Regression(X_train, X_test, y_train, y_test, encoders):
    ...
    plt.subplot(2, 1, 1)
    sns.heatmap(cm, annot=True, fmt="d", xticklabels=encoders["Target"].classes_,
                yticklabels=encoders["Target"].classes_)
    plt.ylabel("Real value")
    plt.xlabel("Predicted value")
    print
    "F1 score: %f" % skl.metrics.f1_score(y_test, y_pred)
...
if __name__ == "__main__":
    ...
    #trainSet, testSet = split_Data_To_Test_And_Train_Sets(encoded_data)
    #print(str("Split data to 2 part : \n\tTest Set  :  " + str(len(testSet))) + "\n\tTrain Set :  " + str(len(trainSet)))
    X_train, X_test, y_train, y_test = split_Data_To_Test_And_Train_Sets(encoded_data)
    logical_Regression(X_train, X_test, y_train, y_test, encoders)
